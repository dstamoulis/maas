import torch
import maas.ext.maas.scripts.optimized.VeriThoughts.test.template.prompt as prompt_custom
import maas.ext.maas.scripts.optimized.VeriThoughts.test.template.operator as operator
from maas.ext.maas.scripts.optimized.VeriThoughts.test.template.operator_registry import operator_mapping, operator_names
...
class Workflow:
    ...
    async def __call__(self, problem: str, log_path: str):
        ...
        if False: # TODO! REMOVE! for layer_idx, selected_names in enumerate(selected_names_layers):
            for op_name in selected_names:
                ...
                if op_name in ["Generate", "GenerateCoT"]:
                    result = await selected_operator(problem=problem, instruction="")
        ...
        test_result = await self.test_operator(problem=problem, solution=current_solution)

        if test_result['result']:
            return test_result['solution'], self.llm.cost_manager.total_cost, sum_log_prob
        else:
            assert 1==0 # TODO! Custom Code?
            new_solution = await self.custom_code_generate(problem=problem, instruction=prompt_custom.IMPROVE_CODE_PROMPT)
